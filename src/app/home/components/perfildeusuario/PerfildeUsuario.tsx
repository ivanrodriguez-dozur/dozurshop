...
export default function PerfilUsuario() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Perfil & stats
      const [p, s] = await Promise.all([loadProfile(), fetchStats()]);
      setProfile(p);
      setStats(s);

      // Mostrar/ocultar coins desde DB si existe bandera
      if (p?.coins_hidden === true) setShowCoins(false);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editName !== profile?.nickname) {
        await updateNickname(editName);
      }
      if (editHalo !== profile?.halo_color) {
        await updateHaloColor(editHalo);
      }
      if (editAvatar && editAvatar !== profile?.avatar_url && fileInputRef.current?.files?.[0]) {
        const url = await updateAvatar(fileInputRef.current.files[0]);
        setEditAvatar(url); // Actualiza la previsualización local
      }
            ...
            {editMode ? (
              <>
                <label htmlFor="avatar-upload" className="w-full h-full flex items-center justify-center cursor-pointer group-hover:opacity-80 transition">
                  {editAvatar ? (
                    <img src={editAvatar} alt="avatar" className="object-cover w-full h-full" />
                  ) : (
