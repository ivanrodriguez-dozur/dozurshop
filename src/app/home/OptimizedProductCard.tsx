...
import { buildSupabasePublicUrl } from '../../lib/resolveImageUrl';
      ...
      <div style={{
        ...
        fontSize: 16,
        zIndex: 2
      }}>${product.price.toLocaleString("es-CO")}</div>
      <div style={{marginTop: 12, fontWeight: 600, color: "#fff", fontSize: 16}}>{product.name}</div>
      {product.description && (
        <div style={{marginTop: 6, color: "#bbb", fontSize: 13, minHeight: 32, overflow: "hidden", textOverflow: "ellipsis"}}>{product.description}</div>
      )}
