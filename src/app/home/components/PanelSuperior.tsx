...
function PanelSuperior(props) {
	...
	const { products = [], popularProducts = [] } = useProducts ? useProducts() : { products: [], popularProducts: [] };
	...
	// Manejar cambios en el input de nickname
	...
	const handleAuthSubmit = async (e) => {
		...
		setTimeout(() => {
			if (isLogin) {
				setUser({ email, user_metadata: { avatar_url: avatars[avatarIndex] } });
				setAuthSuccess("¡Bienvenido!");
				setShowUserModal(false);
			} else {
				setUser({ email, user_metadata: { avatar_url: avatars[avatarIndex] } });
				setAuthSuccess("¡Cuenta creada!");
				setShowCelebration(true);
				setShowAvatarAnim(true);
				setTimeout(() => setShowAvatarAnim(false), 1200);
				setTimeout(() => {
		...
		<>
				...
				<div className="fixed inset-0 z-[9999] pointer-events-none">
					...
					<div className="absolute inset-0 overflow-hidden">
						{[...Array(30)].map((_, i) => {
							...
							return (
								<svg key={i} className="absolute animate-confetti" style={{ left: `${left}%`, top: `${top}%`, width: size, height: size, transform: `rotate(${rotate}deg)` }} viewBox="0 0 32 32">
									<circle cx="16" cy="16" r="12" fill={color} opacity="0.8" />
									<text x="16" y="21" textAnchor="middle" fontSize="18" fill="#fff" fontWeight="bold">★</text>
								</svg>
							);
						})}
					</div>
					{/* Avatar animado */}
					{showAvatarAnim && (
						<div className="fixed left-1/2 top-1/2 z-[10000] flex items-center justify-center" style={{ transform: 'translate(-50%,-50%)' }}>
							<img
			...
			<header className="sticky top-0 z-20 flex items-center justify-between px-4 py-3" style={{ background: 'transparent', border: 'none', boxShadow: 'none' }}>
				...
				<div className="flex items-center gap-3">
					<span className="font-bold text-lg" style={{ color: '#000000ff' }}>{props.pageTitle}</span>
				</div>
		<div className="flex gap-4 items-center">
				{/* Lupa de búsqueda */}
				<button aria-label="Buscar" onClick={() => setShowSearch(true)} className="relative focus:outline-neon cursor-pointer">
					<FaSearch className="w-6 h-6" style={{ color: '#000000ff' }} />
				</button>
						{/* Campana de notificaciones con contador */}
						<button aria-label="Notificaciones" onClick={handleOpenNotifications} className="relative focus:outline-neon cursor-pointer">
							...
							{notifications.some(n => !n.leida) && (
								<span className="absolute -top-1 -right-1 bg-red-600 text-white text-xs rounded-full px-1.5 py-0.5 font-bold">
									...
									{/* Avatar usuario */}
									<button aria-label="Usuario" onClick={() => setShowUserModal(true)} className="relative focus:outline-neon cursor-pointer">
										{user ? (
											<img
												src={user.user_metadata?.avatar_url || avatars[avatarIndex]}
												alt="avatar"
												className="w-8 h-8 rounded-full border-2 border-[#b5ff00] object-cover"
											/>
										) : (
											<FaUserCircle className="w-8 h-8" style={{ color: '#000000ff' }} />
										)}
									</button>
						{/* Modal usuario */}
											{showUserModal && (
												<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 w-screen h-screen">
													<div className="absolute inset-0 z-40" onClick={() => setShowUserModal(false)} />
													<div className="relative z-50">
														<button
															className="absolute top-2 right-2 text-gray-500 hover:text-gray-800 text-2xl bg-white rounded-full shadow p-1"
															onClick={() => setShowUserModal(false)}
															aria-label="Cerrar usuario"
															style={{zIndex: 10}}
														>×</button>
														<UserDashboard userData={user ? {
															avatar_url: user.user_metadata?.avatar_url || avatars[avatarIndex],
															nickname: nickname,
															xp: 0, // TODO: conecta con XP real
															booms: 0, // TODO: conecta con booms reales
															videos: 0, // TODO: conecta con videos vistos reales
															time: 0 // TODO: conecta con tiempo real
														} : undefined} />
													</div>
												</div>
											)}
